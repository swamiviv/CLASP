...
from pixel2style2pixel.datasets.images_dataset import RGBSuperResGeneratedDataset
from pixel2style2pixel.training.coach import Coach
from pixel2style2pixel.utils import common, train_utils
# QR specific imports
from quantile_helpers import gaussian_regression_loss_fn

_STYLE_SPACE_MIN = -200.
_STYLE_SPACE_MAX = 200.
_DISENT_DATA = [
...
class Trainer_gaussian_encoder(Coach):
    def __init__(self, opts):
        ...
        self.gaussian_nll_loss = gaussian_regression_loss_fn
        ...
        if self.opts.checkpoint_path is not None:
            ...
            self.net = psp_encoders.SimpleResnetEncoder_gaussian_output(50, mode='ir_se', opts=self.opts).to(
                self.device)
            ckpt_filt = {k: v for k, v in ckpt['state_dict'].items()}
            self.net.load_state_dict(ckpt_filt, strict=True)

        else:
            self.net = psp_encoders.SimpleResnetEncoder_gaussian_output(
                50, mode='ir_se', opts=self.opts).to(self.device)
        ...
        mean_sv, min_sv, max_sv, std_sv = self.pretrained_psp_net.decoder.mean_style_space_latent(
            latent_dim=512, style_dims=self.style_space_dims
        )

        self.net.style_space_latent_avg = mean_sv
        self.net.style_space_latent_min = min_sv
        self.net.style_space_latent_max = max_sv
        self.net.style_space_latent_std = std_sv
    ...
    def parse_and_log_images(self, x, y, y_hat, y_hat_lq, y_hat_uq, title, subscript=None, id_logs=None,
        ...
        for i in range(display_count):
            cur_im_data = {
                ...
                'output_face: mean': common.tensor2im(y_hat[i]),
                'output_face: 1-sigma': common.tensor2im(y_hat_lq[i]),
                'output_face: 2-sigma': common.tensor2im(y_hat_uq[i]),
            }
    ...
    def calc_loss(self, true_latents, predicted_latents, gt_images, inputs):
        ...
        #merged_predictions = torch.cat(predicted_latents, axis=0)

        mean_prediction = predicted_latents[:, 0, :]
        var_prediction = predicted_latents[:, 1, :]
        random_sample  = torch.randn_like(var_prediction)
        nhood_prediction = mean_prediction + torch.sqrt(var_prediction) * random_sample

        if self.opts.id_lambda > 0 or self.opts.lpips_lambda > 0:
            yhats, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
                latent_codes=torch.randn((nhood_prediction.shape[0], 512)), style_vectors=self.stylespace_flat_to_list(nhood_prediction),
                input_is_latent=True,
                randomize_noise=False, return_latents=True, modulate=False
            )
            yhats = self.face_pool(yhats)
            if self.opts.id_lambda > 0:
                id_loss = self.id_loss.forward_simple(yhats, gt_images, masks=None)
            else:
                lpips_loss = self.lpips_loss(yhats, gt_images)
        #
        ...
        #gaussian_nll_loss = self.gaussian_nll_loss(pred=predicted_latents, target=true_latents)
        latent_loss = self.latent_loss(input=mean_prediction, target=true_latents)
        total_loss = latent_loss + self.opts.id_lambda * id_loss + self.opts.lpips_lambda * lpips_loss + self.opts.w_norm_lambda * w_norm_loss
        ...
        loss_dict['latent_loss'] = float(latent_loss)
    ...
    def add_mean_vector(self, latents):
        latents += self.net.style_space_latent_avg
        return latents
    ...
    def normalize_latents(self, latents, mode='mean'):
        #latents = (latents - self.net.style_space_latent_avg.unsqueeze(0)) / (self.net.style_space_latent_std.unsqueeze(0) + 1e-6)
        latents = latents / (_STYLE_SPACE_MAX)
        return latents

    def unnormalize_latents(self, latents, mode='mean'):
        #latents = (latents * self.net.style_space_latent_std.unsqueeze(0)) + self.net.style_space_latent_avg.unsqueeze(0)
        latents = latents * (_STYLE_SPACE_MAX + 1e-6)
        return latents
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                y = self.face_pool(y)
                predictions = self.net.forward(x)
                predictions[:, 0, :] += self.net.style_space_latent_avg
                ...
                if self.global_step % self.opts.image_interval == 0 or (
                    ...
                    with torch.no_grad():
                        predicted_sv_mean = self.stylespace_flat_to_list(predictions[:, 0, :])
                        predicted_sv_upper_one_sigma = self.stylespace_flat_to_list(
                            predictions[:, 0, :] + torch.sqrt(predictions[:, 1, :])
                        )
                        predicted_sv_lower_one_sigma = self.stylespace_flat_to_list(
                            predictions[:, 0, :] - torch.sqrt(predictions[:, 1, :])
                        )
                        true_sv_to_decode = self.stylespace_flat_to_list(true_sv)

                        fake_y_hat, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
                            latent_codes=torch.randn_like(true_w[:, 0, :]), style_vectors=predicted_sv_mean,
                            input_is_latent=True, randomize_noise=False, return_latents=True, modulate=False
                        )
                        fake_y_hat_one_sigma, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
                            latent_codes=torch.randn_like(true_w[:, 0, :]), style_vectors=predicted_sv_upper_one_sigma,
                            input_is_latent=True, randomize_noise=False, return_latents=True, modulate=False
                        )
                        fake_y_hat_two_sigma, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
                            latent_codes=torch.randn_like(true_w[:, 0, :]), style_vectors=predicted_sv_lower_one_sigma,
                            input_is_latent=True, randomize_noise=False, return_latents=True, modulate=False
                        )
                        fake_y, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
                        ...
                        fake_y_hat = self.face_pool(fake_y_hat)
                        fake_y_hat_one_sigma = self.face_pool(fake_y_hat_one_sigma)
                        fake_y_hat_two_sigma = self.face_pool(fake_y_hat_two_sigma)
                        fake_y = self.face_pool(fake_y)

                    self.parse_and_log_images(x=x, y=fake_y, y_hat=fake_y_hat, y_hat_lq=fake_y_hat_one_sigma,
                                              y_hat_uq=fake_y_hat_two_sigma, title='images/train/faces', display_count=2)
                ...
                if self.opts.use_wandb and batch_idx == 0:
                    self.wb_logger.log_quantile_outputs_to_wandb(
                        x=x, y=fake_y, y_hat=fake_y_hat, y_hat_lq=fake_y_hat_one_sigma, y_hat_uq=fake_y_hat_two_sigma, id_logs=None,
