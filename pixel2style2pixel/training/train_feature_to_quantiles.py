...
class Trainer_embedding_to_quantile(Coach):
    def __init__(self, opts):
        # super().__init__(opts)
        # opts.lpips_lambda = 0
        # opts.id_lambda = 0
        ...
        if self.opts.checkpoint_path is not None:
            ...
            self.net = psp_encoders.FeaturetoStyleSpace_with_quantiles(opts=self.opts).to(
                self.device)
        ...
        mean_sv, _, _ = self.pretrained_psp_net.decoder.mean_style_space_latent(latent_dim=512, style_dims=self.style_space_dims)
    ...
    def calc_loss(self, true_latents, predicted_latents, gt_images, inputs):
        ...
        if self.opts.id_lambda > 0 or self.opts.lpips_lambda > 0:
            ...
            yhats, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
                latent_codes=torch.randn_like(true_latents[0]).repeat(3, 1, 1), style_vectors=merged_predictions, input_is_latent=True,
            ...
            # if self.opts.id_lambda > 0:
            #     id_loss_center = self.id_loss.forward_simple(fake_y_hat, gt_images)
            #     id_loss_lq = self.id_loss.forward_simple(fake_y_hat_lq, gt_images)
            #     id_loss_uq = self.id_loss.forward_simple(fake_y_hat_uq, gt_images)
            #     id_loss = id_loss_center + id_loss_lq + id_loss_uq
            # if self.opts.lpips_lambda > 0:
            #     lpips_loss = self.lpips_loss(fake_y_hat, gt_images) + self.lpips_loss(fake_y_hat_lq, gt_images) + self.lpips_loss(fake_y_hat_uq, gt_images)

        if self.opts.w_norm_lambda > 0:
            if self.opts.start_from_latent_avg:
                latent_mean_diff = torch.cat(predicted_spv, axis=-1).squeeze() - torch.cat(self.net.style_space_latent_avg, axis=-1).squeeze().unsqueeze(0)
                ...
                latent_mean_diff = torch.cat(predicted_spv, axis=-1).squeeze() - torch.cat(self.net.style_space_latent_avg, axis=-1).squeeze().unsqueeze(0)
                latent_mean_diff_uq = torch.cat(predicted_spv_uq, axis=-1).squeeze() - torch.cat(self.net.style_space_latent_avg, axis=-1).squeeze().unsqueeze(0)
                latent_mean_diff_lq = torch.cat(predicted_spv_lq, axis=-1).squeeze() - torch.cat(self.net.style_space_latent_avg, axis=-1).squeeze().unsqueeze(0)
        ...
        quantile_loss_inputs = (
            predicted_spv_with_quantiles, torch.cat(true_latents, axis=-1).squeeze(), quantile_loss_mask,
        )
        quantile_loss, mse_loss = self.quantile_loss(quantile_loss_inputs)
        total_loss = quantile_loss + mse_loss + self.opts.w_norm_lambda * w_norm_loss + self.opts.id_lambda * id_loss
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                x, y = x.to(self.device).float(), y.to(self.device).float()
                y = self.face_pool(y)
                encoded_latents = self.pretrained_psp_net.encoder(x)
                encoded_latents_with_avg = encoded_latents + self.pretrained_latent_avg.unsqueeze(0).repeat(
                    encoded_latents.shape[0], 1, 1)
                _, _, encoded_style_vectors = self.pretrained_psp_net.decoder(
                    [encoded_latents_with_avg], input_is_latent=True, randomize_noise=False,
                    return_latents=False,
                )
                predicted_spv_lq, predicted_spv, predicted_spv_uq = self.net.forward(encoded_style_vectors)
                if self.opts.start_from_latent_avg:
                ...
                true_latents = encoded_latents
                true_style_vectors = encoded_style_vectors
    ...
    def add_mean_vector(self, latents):
        ...
        for idx in range(len(latents)):
            norm_latents[idx] = (latents[idx] + self.net.style_space_latent_avg[idx])
        return norm_latents
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                true_sv = self.stylespace_flat_to_list(true_sv)
                predicted_sv_lq, predicted_sv, predicted_sv_uq = self.net.forward(encoded_style_vectors)
                if self.opts.start_from_latent_avg:
                ...
                if self.global_step % self.opts.image_interval == 0 or (
                    ...
                    with torch.no_grad():
                        ...
                        fake_y, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
                            latent_codes=torch.randn_like(true_w[:, 0, :]), style_vectors=true_sv,
                            input_is_latent=True, randomize_noise=False, return_latents=True, modulate=False
                        )
                        fake_y_hat = self.face_pool(fake_y_hat)
                        fake_y_hat_lq = self.face_pool(fake_y_hat_lq)
