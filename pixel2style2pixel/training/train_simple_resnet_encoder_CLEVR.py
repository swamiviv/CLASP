...
from pixel2style2pixel.datasets.images_dataset import CLEVRSuperResRealGeneratedDataset, CLEVRSuperResGeneratedDataset, RGBSuperResGeneratedDataset
from criteria.lpips.lpips import LPIPS_sg2
...
_CLEVR_DISENT_DIMS=np.array([0, 1, 2, 6, 7, 9, 10, 25, 27, 28, 29, 34, 38, 41, 45, 61, 63, 66, 68, 72,
                             161, 162, 166, 181, 184, 186, 188, 192, 194, 197, 198, 200, 201, 202])

# For pretrained model loading
def get_keys(d, name):
    if 'state_dict' in d:
        d = d['state_dict']
    d_filt = {k: v for k, v in d.items() if k[:len(name)] == name}
    return d_filt
...
class Trainer_direct_quantile_SimpleResnetEncoder(Coach):
    def __init__(self, opts):
        ...
        self.opts.n_style_space_vectors = 18

        self.id_loss = None
        if self.opts.id_lambda > 0:
            self.id_loss = id_loss.IDLoss().to(self.device).eval()
        if self.opts.lpips_lambda > 0:
            self.lpips_loss = LPIPS_sg2(device=self.device).to(self.device).eval()
        lower_q = float(self.opts.alpha * 0.5)
        self.quantile_loss = quantile_loss_helper(params={'q_lo': lower_q, 'q_hi': 1-lower_q})
        self.mse_loss = torch.nn.L1Loss().to(self.device).eval()

        if self.opts.use_wandb:
            from pixel2style2pixel.utils.wandb_utils import WBLogger
            self.wb_logger = WBLogger(self.opts)

        # Initialize pretrained generator
        ckpt = torch.load(opts.pretrained_model_path)
        self.pretrained_G = Generator(128, 12, 4, channel_multiplier=1, modulation_activation=True)
        self.pretrained_G.load_state_dict(get_keys(ckpt['g_ema'], ''), strict=True)
        self.pretrained_G = self.pretrained_G.eval().to(self.device)
        print('Pretrained generator successfully loaded!')

        # Initialize network
        self.list_of_style_space_dims = np.asarray([0, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12])
        #self.list_of_style_space_dims = np.asarray([0, 512, 512, 512, 512, 512, 512, 512, 512, 512, 512, 512, 512, 256, 256, 256, 128, 128, 128, 64, 64])
        ...
        if self.opts.checkpoint_path is not None:
            ...
            self.net = psp_encoders.SimpleResnetEncoder_with_quantiles(18, mode='ir_se', style_dims=np.hstack(self.list_of_style_space_dims).sum(), opts=self.opts).to(
                self.device)
        ...
        self.net.style_space_dims = np.hstack(self.list_of_style_space_dims).sum()
        self.disentangled_dims, self.disentangled_dims_mask = self.get_disentangled_dims()
        print(f' # dims optimized for: {self.disentangled_dims_mask.sum()}')
        self.quantile_loss_mask = torch.ones(self.list_of_style_space_dims.sum()).unsqueeze(0).repeat(self.opts.batch_size, 1)
        if self.opts.use_quantile_loss_mask:
            self.quantile_loss_mask = self.disentangled_dims_mask.unsqueeze(0).repeat(self.opts.batch_size, 1)

        style_dims_cumsum = np.cumsum(self.list_of_style_space_dims)
        self.latent_dim = 12

        mean_sv, min_sv, max_sv, std_sv = self.pretrained_G.mean_style_space_latent(latent_dim=self.latent_dim, style_dims=self.net.style_space_dims)
        self.net.latent_avg = mean_sv
        ...
        if self.opts.experiment_type == 'super_res':
            ...
            probs = [float(f) for f in self.opts.resize_probabilities.split(",")]
            ...
            transform_source = transforms.Compose([
                transforms.Resize((128, 128)),
                augmentations.BilinearResize(factors=factors, probabilities=probs),
                transforms.Resize((128, 128)),
                transforms.ToTensor(),
                transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])])
            self.transform_source = transform_source
            self.transform_target = transform_gt_train
            self.transform_source_test = transform_gt_train
            self.transform_target_test = transform_gt_train
        else:
    ...
    def parse_and_log_images(self, x, y, y_hat, y_hat_lq, y_hat_uq, title, subscript=None, id_logs=None,
        ...
        for i in range(display_count):
            cur_im_data = {
                ...
                'output: lower': common.tensor2im(y_hat_lq[i]),
                'output: central': common.tensor2im(y_hat[i]),
                'output: upper': common.tensor2im(y_hat_uq[i]),
            }
    ...
    def configure_datasets(self):
        ...
        print(f'Real Dataset path: {self.opts.real_train_data_path}')
        print(f'Generated Dataset path: {self.opts.generated_train_data_path}')
        train_dataset = CLEVRSuperResGeneratedDataset(
            generated_db_path=self.opts.generated_train_data_path,
            image_dir_prefix=None,
            source_transform=self.transform_source,
            target_transform=self.transform_target,
            style_vector_ndim=self.net.style_space_dims,
            wplus_shape=(12, self.latent_dim),
        )
        val_dataset = RGBSuperResGeneratedDataset(
            db_path=self.opts.generated_train_data_path,
            source_transform=self.transform_source,
            target_transform=self.transform_target,
        )
    ...
    def get_disentangled_dims(self, disent_data=_CLEVR_DISENT_DIMS):
        disent_dims = disent_data
        style_dims_cumsum = np.cumsum(self.list_of_style_space_dims)
    ...
    def calc_loss(self, true_latents, predicted_latents, gt_images, inputs):
        ...
        if self.opts.id_lambda > 0 or self.opts.lpips_lambda > 0:
            yhats, _ = self.pretrained_G.forward_with_style_vectors(
        ...
        if self.opts.w_norm_lambda > 0:
            latent_mean_diff = predicted_spv - self.net.style_space_latent_avg
            # latent_mean_diff_uq = predicted_spv_uq - self.net.style_space_latent_avg
            # latent_mean_diff_lq = predicted_spv_lq - self.net.style_space_latent_avg
        ...
        quantile_loss_inputs = (
            valid_predicted_spv_with_quantiles, valid_true_latents.squeeze(),
            self.disentangled_dims_mask.unsqueeze(0).repeat(valid_true_latents.shape[0], 1),
        )
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            if batch_idx == 0:
                with torch.no_grad():
                    ...
                    fake_y, _ = self.pretrained_G.decoder.forward_with_style_vectors(
            ...
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.global_step % self.opts.image_interval == 0 or (
                    ...
                    with torch.no_grad():
                        ...
                        fake_y_hat, _ = self.pretrained_G.forward_with_style_vectors(
                            latent_codes=torch.randn_like(true_w[:, 0, :]), style_vectors=predicted_sv_to_decode,
                            input_is_latent=True, randomize_noise=False, return_latents=True, modulate=False
                        )
                        fake_y_hat_lq, _ = self.pretrained_G.forward_with_style_vectors(
                            latent_codes=torch.randn_like(true_w[:, 0, :]),
                            style_vectors= self.stylespace_flat_to_list(predicted_sv_lq),
                            input_is_latent=True, randomize_noise=False, return_latents=True, modulate=False
                        )
                        fake_y_hat_uq, _ = self.pretrained_G.forward_with_style_vectors(
                            latent_codes=torch.randn_like(true_w[:, 0, :]),
                            style_vectors=self.stylespace_flat_to_list(predicted_sv_uq),
                            input_is_latent=True, randomize_noise=False, return_latents=True, modulate=False
                        )
                        fake_y, _ = self.pretrained_G.forward_with_style_vectors(
