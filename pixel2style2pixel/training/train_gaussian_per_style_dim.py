...
from pixel2style2pixel.datasets.images_dataset import RGBInpaintGeneratedDataset, RGBSegMaskDataset, RGBSuperResGeneratedDataset
...
# QR specific imports
from quantile_helpers import quantile_loss_helper, gaussian_regression_loss_fn
...
class Trainer_gaussian_style_vector(Coach):
    def __init__(self, opts):
        ...
        mean_sv, min_sv, max_sv = self.pretrained_psp_net.decoder.mean_style_space_latent(
    ...
    def calc_loss(self, true_latents, predicted_latents, gt_images, inputs):
        ...
        #         latent_codes=torch.randn((merged_predictions.shape[0], 512)), style_vectors=self.stylespace_flat_to_list(merged_predictions),
        #         input_is_latent=True,
        #         randomize_noise=False, return_latents=True, modulate=False
        #     )
        #     yhats = self.face_pool(yhats)
        #     if self.opts.id_lambda > 0:
        #         id_loss = self.id_loss.forward_simple(yhats, gt_images.repeat(3, 1, 1, 1), masks=input_masks.unsqueeze(1).repeat(3, 1, 1, 1))
        #     else:
        ...
        total_loss = gaussian_nll_loss + self.opts.id_lambda * id_loss + self.opts.lpips_lambda * lpips_loss + self.opts.w_norm_lambda * w_norm_loss
        ...
        loss_dict['gaussian_nll_loss'] = float(gaussian_nll_loss)
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                y = self.face_pool(y)
                predictions = self.net.forward(x)
                predictions[:, 0, :] += self.net.style_space_latent_avg.unsqueeze(0).unsqueeze(0).repeat((1, 3, 1))
                ...
                if self.global_step % self.opts.image_interval == 0 or (
                    ...
                    with torch.no_grad():
                        ...
                        predicted_sv_one_sigma = self.stylespace_flat_to_list(
                            predictions[:, 0, :] + torch.sqrt(predictions[:, 1, :])
                        )
                        predicted_sv_two_sigma = self.stylespace_flat_to_list(
                            predictions[:, 0, :] + 2 * torch.sqrt(predictions[:, 1, :])
                        )
                        ...
                        fake_y_hat_one_sigma, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
                            latent_codes=torch.randn_like(true_w[:, 0, :]), style_vectors=predicted_sv_one_sigma,
                            input_is_latent=True, randomize_noise=False, return_latents=True, modulate=False
                        )
                        fake_y_hat_two_sigma, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
                            latent_codes=torch.randn_like(true_w[:, 0, :]), style_vectors=predicted_sv_two_sigma,
                            input_is_latent=True, randomize_noise=False, return_latents=True, modulate=False
                        )
                        fake_y, _ = self.pretrained_psp_net.decoder.forward_with_style_vectors(
