...
from pixel2style2pixel.datasets.images_dataset import RGBInpaintGeneratedDataset, RGBSegMaskDataset, RGBSuperResDataset
...
class Trainer_direct_quantile_SimpleResnetEncoder(Coach):
    def __init__(self, opts):
        ...
        # Init transforms
        transform_obj = transforms_config.DirectInpaintTransforms(opts)
    ...
    def configure_datasets(self):
        ...
        train_dataset = RGBInpaintGeneratedDataset(
            db_path=self.opts.train_data_path,
            source_transform=self.transform_source,
            target_transform=self.transform_target,
            mask_thresholds={'low': self.opts.mask_threshold_low, 'high': self.opts.mask_threshold_high},
        )
        val_dataset = RGBInpaintGeneratedDataset(
            db_path=self.opts.train_data_path,
            source_transform=self.transform_source,
            target_transform=self.transform_target,
            mask_thresholds={'low': 0.1, 'high': 0.3},
        )
    ...
    def calc_loss(self, true_latents, predicted_latents, gt_images, inputs):
        ...
        predicted_spv_with_quantiles = torch.cat(
            (
                predicted_spv_lq.unsqueeze(1),
                predicted_spv.unsqueeze(1),
                predicted_spv_uq.unsqueeze(1),
            ),
            axis=1,
        )
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                y = self.face_pool(y)
                predictions = self.net.forward(x)
                predictions += self.net.style_space_latent_avg.unsqueeze(0).unsqueeze(0).repeat((1, 3, 1))
