...
class DirectSuperResTransforms(TransformsConfig):

	def __init__(self, opts):
		super(DirectSuperResTransforms, self).__init__(opts)
	...
	def get_transforms(self):
		...
		factors = [int(f) for f in self.opts.resize_factors.split(",")]
		print("Performing down-sampling with factors: {}".format(factors))
		if self.opts.resize_probabilities is not None:
			probs = [float(f) for f in self.opts.resize_probabilities.split(",")]
			print("Performing down-sampling with probabilities: {}".format(probs))
		...
		transforms_dict = {
			...
			'transform_source': transforms.Compose([
				...
				transforms.ToTensor(),
				transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])]),
			'transform_source_test': transforms.Compose([
				transforms.Resize((256, 256)),
				augmentations.BilinearResize(factors=factors),
				transforms.Resize((256, 256)),
				transforms.ToTensor(),
...
class DirectInpaintTransforms(TransformsConfig):

	def __init__(self, opts):
		super(DirectInpaintTransforms, self).__init__(opts=None)
