...
class TrainOptions_quantile_regression:
	...
	def initialize(self):
		self.parser.add_argument('--exp_dir', type=str, help='Path to experiment output directory')
		self.parser.add_argument('--real_train_data_path', type=str, help='Real Train dataset path')
		self.parser.add_argument('--generated_train_data_path', type=str, help='Generated Train dataset path')
		self.parser.add_argument('--generated_data_dir_prefix', type=str, help='Generated Train dataset path')
		self.parser.add_argument('--val_data_path', type=str, help='Validation dataset path')
		self.parser.add_argument('--encoder_type', default='GradualStyleEncoder_with_quantiles', type=str, help='Which encoder to use')
		self.parser.add_argument('--input_nc', default=3, type=int, help='Number of input image channels to the psp encoder')
		self.parser.add_argument('--label_nc', default=0, type=int, help='Number of input label channels to the psp encoder')
		self.parser.add_argument('--pretrained_psp_resolution', default=256, type=int, help='Size of pretrained psp encoder')
		self.parser.add_argument('--output_size', default=1024, type=int, help='Output size of generator')
		...
		self.parser.add_argument('--image_interval', default=250, type=int, help='Interval for logging train images during training')
		self.parser.add_argument('--board_interval', default=50, type=int, help='Interval for logging metrics to tensorboard')
		...
		self.parser.add_argument('--mse', action='store_true', help='Whether to add mse in QR loss')
		self.parser.add_argument('--no_quantile_image_loss', action='store_true', help='whether to apply image loss to quantiles')
		self.parser.add_argument('--pretrained_model_path', type=str, help='pretrained psp model path')
		self.parser.add_argument('--experiment_type', type=str, help='Name of the experiment')
		self.parser.add_argument('--use_style_vector_average', action='store_true', help='Whether to normalize with style space latent average')
		self.parser.add_argument('--use_quantile_loss_mask', action='store_true',
								 help='Whether to mask quantile loss')
		self.parser.add_argument('--normalize_sv', action='store_true', help='Whether to normalize style vectors')
		self.parser.add_argument('--alpha', default=0.1, type=float, help='Quantile to learn')

		# Inpainting arguments
		self.parser.add_argument('--mask_threshold_low', default=0.3, type=float, help='Lower threshold for masking')
		self.parser.add_argument('--mask_threshold_high', default=0.95, type=float, help='Upper threshold for masking')


		# arguments for weights & biases support
		self.parser.add_argument('--use_wandb', action="store_true", help='Whether to use Weights & Biases to track experiment.')
		self.parser.add_argument('--wandb_run_name', type=str, help='run name')

		# arguments for super-resolution
		self.parser.add_argument('--resize_factors', type=str, default=None, help='For super-res, comma-separated resize factors to use for inference.')
		self.parser.add_argument('--resize_probabilities', type=str, default=None, help='For super-res, comma-separated resize probs to use for inference.')
