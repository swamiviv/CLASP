...
class BilinearResize(object):
	def __init__(self, factors=[1, 2, 4, 8, 16, 32], probabilities=None):
		self.factors = factors
		self.p = probabilities
		print(f'BilinearResize Initialized with factors: {factors} and probs: {probabilities}')

	def __call__(self, image):
		if self.p is None:
			factor = np.random.choice(self.factors, size=1)[0]
		else:
			factor = np.random.choice(self.factors, size=1, p=self.p)[0]
